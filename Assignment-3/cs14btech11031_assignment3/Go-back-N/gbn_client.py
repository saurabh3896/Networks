...
def corrupt(packet) :
    # method to perform a checksum check for the new packet made and the one sent in the packet
    seq_num, terminate, check_sum, length = struct.unpack('HHHL', packet[:16])
    # make a new packet and compare its checksum with one arrived
        ...
        if seq_num == expectedseqnum and not corrupt(data) :
            ...
            if data[-1] == "|" and data[-2] == "|" :
                ...
                try :
                    ...
                    pass
                # create a file depending on the information sent in the first packet
